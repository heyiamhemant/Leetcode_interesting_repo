...
// @lc code=start
class TimeMap {
    std::unordered_map<string, std::vector<std::pair<int , string>> > Map; // timestamp is already sorted
public:
    TimeMap() {
    ...
    string get(string key, int timestamp) {
        ...
        auto &values = Map[key];
        std::string result;
        // binary search to find greatest entry with timestamp <= timestamp
        int low = 0;
        ...
        while(low <= high) {
            ...
            string value = values[mid].second;
            //cout << "time : " << t << " value : " << value << endl;
            if(t <= timestamp) {
                low = mid + 1; // look if greater timestamp which satisfies condition exists
                result = value;
            } else high = mid - 1;
        }
        return result;
