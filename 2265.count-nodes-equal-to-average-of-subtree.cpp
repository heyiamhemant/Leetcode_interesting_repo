...
class Solution {
public:
    void calculateAverage(TreeNode* root, int &sum, int &n, int &res) {
        if(!root) return;
        int sumleft = 0;
        int nleft = 0;
        int sumright = 0;
        int nright = 0;
        calculateAverage(root->left, sumleft, nleft, res);
        calculateAverage(root->right, sumright, nright, res);
    ...
    int averageOfSubtree(TreeNode* root) {
        ...
        int sum = 0, n = 0;
        int res = 0;
        calculateAverage(root, sum, n, res);
