...
public:
    bool isValidSudoku(vector<vector<char>>& board) {
        vector<set<int>> rowMap(9, std::set<int> ());
        vector<set<int>> colMap (9, std::set<int> ());
        vector<set<int>> boxMap (9, std::set<int> ());
        ...
        for (int i = 0; i < 9; i++) {
            for (int j = 0; j < 9; j++) {
                if (board[i][j] != '.') {
                    ...
                    if (rowMap[i].find(num) != rowMap[i].end() ||
                        colMap[j].find(num) != colMap[j].end() ||
