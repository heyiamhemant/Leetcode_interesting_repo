...
class Solution {
public:
    void traverseAndLink(Node* node, std::unordered_map<Node*, Node*> &Map) {
        if(!node) return;
        for(Node* neighbor : node->neighbors)
        {
            if(Map.contains(neighbor)) // already traversed, just link
                Map[node]->neighbors.push_back(Map[neighbor]);
            else {
                Map[neighbor] = new Node(neighbor->val);
                Map[node]->neighbors.push_back(Map[neighbor]);
                traverseAndLink(neighbor, Map);
            }
        }
    }
