...
class Solution {
public:
    void ignoreChar(string &s , int &it, char c) {
        while(it < s.size() && s[it] == c) it++;
    }
    int myAtoi(string s) {
        int polarity = 1;
        int it = 0;
        ignoreChar(s, it, ' '); 
        if(it < s.size() && s[it] == '-') { polarity = -1; it++;}
        else if(it < s.size() && s[it] == '+') { it++; }
        ignoreChar(s, it, '0');
        long long value = 0;
        while(it < s.size() && isdigit(s[it])) {
            value = value * 10 + (s[it++] - '0');
            if(value > INT_MAX) 
                return polarity == 1 ? INT_MAX : INT_MIN;
        }
        return polarity * value;

    }
};
class SolutionNoLongLong {
public:
    void ignoreChar(string &s , int &it, char c) {
        while(it < s.size() && s[it] == c) it++;
    }
    int myAtoi(string s) {
        ...
        while(it < s.size() && isdigit(s[it])) {
            int digit = s[it++] - '0';
            if(value > INT_MAX/10 || value >= INT_MAX/10 && digit > 7) // 21474836 + 8 is overflow
                return polarity == 1 ? INT_MAX : INT_MIN;
